...
""" Problem 1: Shopping Cart Dictionary
...
def add_item(cart: dict, item_name: str, price: float, quantity: int) -> bool:
...
def calculate_total(cart: dict[str, tuple[int, float]]) -> float:
    return sum(price * quantity for price, quantity in cart.values())

def apply_discount(cart: dict[str, tuple[int, float]], discount_percent: int|float):
    if discount_percent < 0 or discount_percent > 100:
        return False
    
    price_mult = 1 - (discount_percent / 100)
    
    for item in cart:
        price, quantity = cart[item]
        cart[item] = (price * price_mult, quantity)
    
    return True



""" Problem 5: Pattern Matching with Regex
...
# Bonus
def validate_time(time_string) -> bool:
    time_pattern = r"(^[0-1][0-9]|2[0-3]):([0-5][0-9])$"
    
    return bool(re.match(time_pattern, time_string))

# Bonus
def find_dates(text) -> list[str]:
...
""" Problem 2: Student Groups with Sets
...
def add_student_to_club(clubs_dict: dict[str, set[str]], club_name: str, student_name: str) -> None:
    if club_name in clubs_dict:
        clubs_dict[club_name].add(student_name)
    else:
        clubs_dict[club_name] = set([student_name])
        print(clubs_dict)

def get_students_in_multiple_clubs(clubs_dict: dict[str, set[str]]) -> set[str]:
    active_students = set()
    
    for club_a in clubs_dict:
        for club_b in clubs_dict:
            if club_a != club_b:
                for student in clubs_dict[club_a] & clubs_dict[club_b]:
                    active_students.add(student)
    
    return active_students

def find_exclusive_members(clubs_dict: dict[str, set[str]], club1_name: str, club2_name: str) -> set[str]:
    return clubs_dict[club1_name] - clubs_dict[club2_name]

# Bonus Question
def merge_clubs(clubs_dict: dict[str, set[str]], old_club: str, new_club: str) -> bool:
    if old_club not in clubs_dict:
        return False
    
    clubs_dict[new_club] = clubs_dict[new_club] | clubs_dict[old_club]
...
def create_score_array(num_students:int , num_exams: int):
    return np.random.randint(60, 101, (num_students, num_exams))

def find_struggling_students(scores: np.ndarray, threshold: int):
    student_averages = np.mean(scores, axis=1)
    return student_averages < threshold

def curve_scores(scores: np.ndarray, bonus_points: int):
    return np.minimum(scores + bonus_points, 100)

def get_exam_statistics(scores: np.ndarray):
    stats_dict = {}
    
    for i, exam in enumerate(scores.transpose()): # transpose array to iterate over columns
        stats_dict[f'exam_{i}'] = {
            'mean': np.mean(exam),
            'min': np.min(exam),
            'max': np.max(exam)
        }
...
def clean_message(message) -> str:
    return ' '.join(message.lower().split()).rstrip('.').rstrip('!').rstrip('?')
...
def expand_abbreviations(message: str) -> str:
    ...
    expanded = ' ' + message + ' ' # jank padding to fix a bug
...
def count_words(message) -> dict[str, int]:
    words = message.split()
...
def censor_numbers(message: str) -> str:
    new = ''
    
    for i, char in enumerate(message):
        if not char.isdigit():
            new += char
        elif not message[i - 1].isdigit() or i == 1:
            new += 'XXX'
...
if __name__ == "__main__":
    print("---- Problem 1: Shopping Cart Dictionary ----")
